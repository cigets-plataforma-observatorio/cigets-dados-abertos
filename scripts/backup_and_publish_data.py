...
df = pd.read_sql(
    "select * from INFORMATION_SCHEMA.VIEWS where table_schema = 'Analytics Layer.Educação'",
    cnxn,
)


def saveDDLOnDisk(item):
    caminho = item["TABLE_SCHEMA"].replace(".", os.sep)
    pathlib.Path(caminho).mkdir(parents=True, exist_ok=True)
    nome_arquivo = item["TABLE_NAME"].replace("\\", "-").replace("/", "-")
...
def readDataFromDremioAndWriteOnDisk(caminho, nome_arquivo):
    with open(os.path.join(caminho, nome_arquivo), "r") as ddl:
        ...
        nomeArquivoCSV = (
            str(nome_arquivo).replace(" ", "_").replace("-", "").replace(",", "")
            + ".csv"
        )
        file = open(nomeArquivoCSV, "w")
...
def updateResourceOnCKAN(nomeArquivoCSV, resourceID, ckanAPIKey):
    with open(nomeArquivoCSV, "rb") as csv:
        request_data = {
            "id": resourceID,  # Deve receber dinamicamente
            "upload": (nomeArquivoCSV, csv, "application/octet-stream"),
        }
        form = encoder.MultipartEncoder(request_data)
        headers = {"Authorization": ckanAPIKey}
        headers["Content-Type"] = form.content_type
        print("Updating resource: " + nomeArquivoCSV)
        response = requests.post(
            f"{ckan_api_url}/resource_patch", headers=headers, data=form
        )
...
for _, item in df.iterrows():
    caminho, nome_arquivo = saveDDLOnDisk(item)

    # Consultando e escrevendo os resultados
    nomeArquivoCSV = readDataFromDremioAndWriteOnDisk(
        caminho=caminho, nome_arquivo=nome_arquivo
    )

    updateResourceOnCKAN(
        nomeArquivoCSV=nomeArquivoCSV,
        ckanAPIKey=ckan_api_key,
        resourceID="4f0c13c9-8fd8-412c-8215-78d290acb7c1",
    )
